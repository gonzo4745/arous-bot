(function () {
  ...
  function pageMainMenu() {
    return function(p) {
      p.appendChild(btnStack([
        { label: '👋 About Arous',         msg: 'About Arous' },
        { label: '👥 Who We Help',         msg: 'Who We Help' },
  ...
  function pageFAQs() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        { label: '❓ What happens during an assessment?',       msg: 'What happens during an assessment?',       fromButton: true },
        { label: '⏱️ How long does an assessment take?',        msg: 'How long does an assessment take?',        fromButton: true },
        { label: '🔍 What kinds of issues can be identified?',  msg: 'What kinds of issues can be identified?',  fromButton: true },
        { label: '🏢 What types of businesses benefit most?',   msg: 'What types of businesses benefit most?',   fromButton: true },
        { label: '💻 Do I need new software?',                  msg: 'Do I need new software?',                  fromButton: true },
        { label: '🤖 Is AI replacing employees?',               msg: 'Is AI replacing employees?',               fromButton: true },
      ]));
      p.appendChild(navRow([
        { label: '🏠 Main Menu', action: function(){ setPage(pageMainMenu()); } },
      ]));
  ...
  var PANEL_RESPONSES = {
    'About Arous': true,
    'Who We Help': true,
    'Business Challenges': true,
  };

  var FAQ_QUESTIONS = {
    'What happens during an assessment?': true,
    'How long does an assessment take?': true,
    'What kinds of issues can be identified?': true,
    'What types of businesses benefit most?': true,
    'Do I need new software?': true,
    'Is AI replacing employees?': true,
  };

  async function sendMessage(fromButton) {
    var text = input.value.trim();
    if (!text) return;
    var isFAQ = FAQ_QUESTIONS[text] || false;
    var isPanelResponse = PANEL_RESPONSES[text] || false;
    var faqLabel = isFAQ ? (displayText || text) : null;
    ...
    try {
      ...
      if (isAssessmentSelection) {
        ...
        var _faqLabel = faqLabel;
        setPage(function(p) {
          if (_faqLabel) p.appendChild(titleBlock(_faqLabel.replace(/^[^\w\s]+\s*/, '')));
          var resp = el('div', { className: 'arous-menu-content' });
          resp.textContent = botText;
          p.appendChild(resp);
          p.appendChild(navRow([
