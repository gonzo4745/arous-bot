(function () {
  ...
  function btnStack(items) {
    ...
    items.forEach(function(b) {
      ...
      btn.onclick = b.action || (function(m, c) {
        return function() { input.value = m; sendMessage(c); };
      })(b.msg, b.context || null);
  ...
  function pageMainMenu() {
    return function(p) {
      p.appendChild(btnStack([
        { label: '\u{1F4C5} Schedule Assessment', action: function(){ setPage(pageAssessmentMenu()); } },
        { label: '\u{1F6E0} Services',            action: function(){ setPage(pageServicesMenu()); } },
        { label: '\u{1F4A1} Business Challenges', msg: 'Business Challenges' },
        { label: '\u{1F4AC} Talk to Daniel',      msg: 'Talk to Daniel' },
      ]));
    };
  }
  ...
  function pageAssessmentMenu() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        { label: '\u{1F50D} Lost Opportunities Assessment', msg: 'I would like to schedule a Lost Opportunities Assessment' },
        { label: '\u{1F4CA} Business Operations Assessment', msg: 'I would like to schedule a Business Operations Assessment' },
      ]));
      p.appendChild(navRow([
        { label: '\u{1F3E0} Main Menu', action: function(){ setPage(pageMainMenu()); } },
      ]));
    };
  }
  ...
  function pageServicesMenu() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        { label: '\u{1F50D} Lost Opportunities Assessment', action: function(){ setPage(pageLostOpportunities()); } },
        { label: '\u{1F4CA} Business Operations Assessment', action: function(){ setPage(pageBusinessOperations()); } },
        { label: '\u{1F916} AI Website Concierge',           action: function(){ setPage(pageAIConcierge()); } },
  ...
  function pageLostOpportunities() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        { label: '\u{1F4C5} Schedule This Assessment', msg: 'I would like to schedule a Lost Opportunities Assessment' },
      ]));
      p.appendChild(navRow([
  ...
  function pageBusinessOperations() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        { label: '\u{1F4C5} Schedule This Assessment', msg: 'I would like to schedule a Business Operations Assessment' },
      ]));
      p.appendChild(navRow([
        { label: '\u2B05 Services',   action: function(){ setPage(pageServicesMenu()); } },
        { label: '\u{1F3E0} Main Menu', action: function(){ setPage(pageMainMenu()); } },
  ...
  function pageAISolutionsMenu() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        ...
        { label: '\u{1F4C6} AI Scheduling Assistant', action: function(){ setPage(pageAISolution('AI Scheduling Assistant', 'Allow customers to request appointments, submit scheduling preferences, and streamline the scheduling process through an intelligent conversational experience.')); } },
        { label: '\u{1F517} CRM Integrations',        action: function(){ setPage(pageAISolution('CRM Integrations',        'Connect your AI solutions with supported CRM platforms to improve lead management, reduce manual entry, and keep customer information synchronized.')); } },
        { label: '\u{1F4F2} Missed Call Text-Back',   action: function(){ setPage(pageAISolution('Missed Call Text-Back',   'Automatically respond to missed phone calls with a personalized text message, helping recover opportunities that might otherwise be lost.\n\nAvailability depends on your existing phone system.')); } },
        { label: '\u{1F4AC} AI SMS Assistant',        action: function(){ setPage(pageAISolution('AI SMS Assistant',        'Provide customers with intelligent two-way text messaging for answering questions, collecting information, and supporting customer communication.\n\nAvailability depends on your existing messaging platform.')); } },
  ...
  function pageAISolution(name, body) {
    return function(p) {
      ...
      p.appendChild(navRow([
        { label: '\u{1F4AC} Talk to Daniel', msg: 'Talk to Daniel', context: 'ai-solution' },
  ...
  async function sendMessage(context) {
    ...
    try {
      ...
      if (scheduleIntent) {
        ...
        // Render response INSIDE the panel so it's cleared on next navigation
        setPage(function(p) {
          var resp = el('div', { className: 'arous-bot' });
          resp.style.cssText = 'max-width:100%;border-radius:4px 14px 14px 14px;';
          resp.innerHTML = renderText(botText);
          p.appendChild(resp);
          p.appendChild(navRow([
            { label: '⬅ AI Solutions', action: function(){ setPage(pageAISolutionsMenu()); } },
