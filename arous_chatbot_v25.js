(function () {
  ...
  function formCheckboxGroup(state, key, options, allowOther, exclusiveOptions) {
    ...
    function toggle(opt, cb) {
      var isExclusive = exclusiveOptions.indexOf(opt) !== -1;
      if (cb.checked) {
        if (isExclusive) {
          // clear every other selection
          selected.length = 0;
          selected.push(opt);
          allCheckboxes.forEach(function(other) {
            if (other.opt !== opt) { other.cb.checked = false; if (other.otherInput) other.otherInput.style.display = 'none'; }
          });
        } else {
          // clear any exclusive selection currently active
          exclusiveOptions.forEach(function(ex) {
            var i = selected.indexOf(ex);
            if (i > -1) selected.splice(i, 1);
          });
          allCheckboxes.forEach(function(other) {
            if (exclusiveOptions.indexOf(other.opt) !== -1) other.cb.checked = false;
          });
    ...
    options.forEach(function(opt) {
      ...
      cb.onchange = function() { toggle(opt, cb); };
      row.appendChild(cb);
      row.appendChild(el('span', { textContent: opt }));
      wrap.appendChild(row);
      allCheckboxes.push({ opt: opt, cb: cb, otherInput: null });
    });

    if (allowOther) {
      var row = el('label', { className: 'arous-form-check-row' });
      var cb = el('input', { type: 'checkbox' });
      cb.checked = selected.indexOf('Other') !== -1;
      ...
      allCheckboxes.push({ opt: 'Other', cb: cb, otherInput: otherInput });
    }
  ...
  // ─── BUSINESS OPERATIONS ASSESSMENT — multi-step questionnaire ───────────
  // NOTE: Q1–Q3 below are PLACEHOLDER questions so the flow is complete end
  // to end. Swap buildFields/validate in pageBOAStep1/2/3 once the real
  // Business Operations questionnaire is finalized — the wizard shell,
  // submission, and confirmation screen do not need to change.
  var boaForm = {};
  ...
  function pageBOAStep0() {
    return buildFormStep({
      ...
      intro: 'Answer a few quick questions so Daniel can review your operations and provide personalized recommendations.',
      buildFields: function(container) {
  ...
  // PLACEHOLDER — replace with real Business Operations questions
  function pageBOAStep1() {
    return buildFormStep({
      step: 1, total: 4,
      title: 'What operational area would you like us to focus on?',
      intro: 'Select all that apply. (Placeholder question)',
      buildFields: function(container) {
        container.appendChild(formCheckboxGroup(boaForm, 'q1', [
          'Workflows & processes', 'Customer experience', 'Technology & tools', 'Team productivity'
        ], true));
      },
      validate: function() {
  ...
  function pageBOAStep2() {
    return buildFormStep({
      step: 2, total: 4,
      title: 'How would you rate your current operational efficiency?',
      intro: '(Placeholder question)',
      buildFields: function(container) {
        container.appendChild(formRadioGroup(boaForm, 'q2', [
          'Excellent', 'Good', 'Needs improvement', 'Struggling', 'Not sure'
        ], false));
      },
      validate: function() {
        if (!boaForm.q2) return 'Please choose one option.';
        return null;
      },
  ...
  function pageBOAStep3() {
    return function(p) {
      p.appendChild(formProgress(3, 4));
      p.appendChild(titleBlock('Is there anything else you\u2019d like Daniel to know about your business?'));
      p.appendChild(contentBlock('Optional.'));
      p.appendChild(formTextareaField(boaForm, 'q3', { placeholder: 'Anything else you\u2019d like to share...' }));

      var errorHolder = el('div', {});
      p.appendChild(errorHolder);

      p.appendChild(btnStack([
        { label: 'Submit Assessment', action: function() {
            setPage(function(pp) {
              pp.appendChild(formProgress(3, 4));
      ...
      p.appendChild(navRow([
        { label: '⬅ Back',      action: function(){ setPage(pageBOAStep2()); } },
        { label: '🏠 Main Menu', action: function(){ setPage(pageMainMenu()); } },
      ]));
    };
  }
  ...
  function pageBOAError() {
    return function(p) {
      ...
      p.appendChild(btnStack([
        { label: 'Try Again', action: function(){ setPage(pageBOAStep3()); } },
      ]));
      p.appendChild(navRow([
        { label: '💬 Talk to Daniel', action: function(){ setPage(pageTalkToDaniel()); } },
        { label: '🏠 Main Menu',      action: function(){ setPage(pageMainMenu()); } },
