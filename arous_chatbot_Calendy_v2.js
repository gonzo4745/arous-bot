(function () {
  ...
  style.textContent = `
    ...
    #arous-cal-close:hover { background: rgba(0,0,0,.16); }
    #arous-cal-body {
      flex: 1;
      overflow-y: auto;
      padding-top: 8px;
  ...
  calOverlay.innerHTML = '<div id="arous-cal-modal"><button id="arous-cal-close" aria-label="Close">&times;</button><div id="arous-cal-body"></div></div>';
  document.body.appendChild(calOverlay);

  var _calScriptLoaded = false;

  function openCalendly() {
    var body = document.getElementById('arous-cal-body');
    // Inject widget div fresh each time so Calendly re-initialises cleanly
    body.innerHTML = '<div class="calendly-inline-widget" data-url="https://calendly.com/hello-arous/30min" style="min-width:320px;height:700px;"></div>';
    if (!_calScriptLoaded) {
      var s = document.createElement('script');
      s.src = 'https://assets.calendly.com/assets/external/widget.js';
      s.async = true;
      document.head.appendChild(s);
      _calScriptLoaded = true;
