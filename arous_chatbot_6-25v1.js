(function () {
  ...
  function showServicesMenu() {
    ...
    addMsg("[Services menu placeholder — update content here]", 'bot');
  ...
  // ─── SERVICES: LOST OPPORTUNITIES ASSESSMENT ──────────────────────────────

  function showLostOpportunitiesDetail() {
    addMsg("[Lost Opportunities Assessment detail placeholder — update content here]", 'bot');

    var btnStack = document.createElement('div');
    btnStack.className = 'arous-btn-stack';

    var scheduleBtn = document.createElement('button');
    scheduleBtn.className = 'arous-btn';
    scheduleBtn.textContent = '📅 Schedule This Assessment';
    scheduleBtn.onclick = function() {
      btnStack.remove();
      nav.remove();
      input.value = 'I would like to schedule a Lost Opportunities Assessment';
      sendMessage();
    };
    btnStack.appendChild(scheduleBtn);
    M.appendChild(btnStack);
    ...
    var backBtn = document.createElement('button');
    backBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    backBtn.textContent = '⬅ Services';
    backBtn.onclick = function() { nav.remove(); btnStack.remove(); showServicesMenu(); };
    var homeBtn = document.createElement('button');
    homeBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    homeBtn.textContent = '🏠 Main Menu';
    homeBtn.onclick = function() { nav.remove(); btnStack.remove(); showMainButtons(); };
  ...
  // ─── SERVICES: BUSINESS OPERATIONS ASSESSMENT ─────────────────────────────

  function showBusinessOperationsDetail() {
    addMsg("[Business Operations Assessment detail placeholder — update content here]", 'bot');
    ...
    scheduleBtn.onclick = function() {
      btnStack.remove();
      nav.remove();
      input.value = 'I would like to schedule a Business Operations Assessment';
      sendMessage();
    };
    btnStack.appendChild(scheduleBtn);
    M.appendChild(btnStack);
  ...
  // ─── SERVICES: AI WEBSITE CONCIERGE ───────────────────────────────────────

  function showAIConciergeDetail() {
    addMsg("[AI Website Concierge detail placeholder — update content here]", 'bot');
    ...
    backBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    backBtn.textContent = '⬅ Services';
    backBtn.onclick = function() { nav.remove(); showServicesMenu(); };
    var homeBtn = document.createElement('button');
    homeBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    homeBtn.textContent = '🏠 Main Menu';
    homeBtn.onclick = function() { nav.remove(); showMainButtons(); };
    nav.appendChild(backBtn);
    nav.appendChild(homeBtn);
    M.appendChild(nav);
    M.scrollTop = M.scrollHeight;
  ...
  // ─── SERVICES: AI SOLUTIONS & IMPLEMENTATIONS ─────────────────────────────

  function showAISolutionsMenu() {
    addMsg("[AI Solutions & Implementations menu placeholder — update content here]", 'bot');
  ...
  // ─── AI SOLUTION PLACEHOLDERS ─────────────────────────────────────────────

  function _aiSolutionDetail(placeholderLabel) {
    addMsg("[" + placeholderLabel + " detail placeholder — update content here]", 'bot');
    var nav = document.createElement('div');
    nav.style.cssText = 'align-self:flex-start; margin-top:4px; display:flex; gap:6px;';
    var talkBtn = document.createElement('button');
    talkBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    talkBtn.textContent = '💬 Talk to Daniel';
    talkBtn.onclick = function() { nav.remove(); input.value = 'Talk to Daniel'; sendMessage(); };
    var backBtn = document.createElement('button');
    backBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    backBtn.textContent = '⬅ AI Solutions';
    backBtn.onclick = function() { nav.remove(); showAISolutionsMenu(); };
    var homeBtn = document.createElement('button');
    homeBtn.style.cssText = 'background:transparent;border:1px solid rgba(167,139,250,.25);color:rgba(167,139,250,.6);border-radius:8px;padding:5px 12px;cursor:pointer;font-size:11px;font-family:sans-serif;';
    homeBtn.textContent = '🏠 Main Menu';
    homeBtn.onclick = function() { nav.remove(); showMainButtons(); };
    nav.appendChild(talkBtn);
    nav.appendChild(backBtn);
    nav.appendChild(homeBtn);
    M.appendChild(nav);
    M.scrollTop = M.scrollHeight;
  ...
  function showWorkflowAutomation()  { _aiSolutionDetail('Workflow Automation'); }
  function showAIScheduling()        { _aiSolutionDetail('AI Scheduling Assistant'); }
  function showCRMIntegrations()     { _aiSolutionDetail('CRM Integrations'); }
  function showMissedCallTextBack()  { _aiSolutionDetail('Missed Call Text-Back'); }
  function showAISMSAssistant()      { _aiSolutionDetail('AI SMS Assistant'); }
  function showAIVoiceAssistant()    { _aiSolutionDetail('AI Voice Assistant'); }
  function showCustomAIAgents()      { _aiSolutionDetail('Custom AI Agents'); }
